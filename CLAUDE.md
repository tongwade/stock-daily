...
- **Weekly (週報)**: `data/weekly/YYYYMMDD-MMDD/` folders are processed by `classify_weekly` → `process_weekly` (買進前20/賣出前20) + `process_volume_avg` (大量與均價 → `{code}_vol.json`). `index.json` gets a `weekly_dates` list alongside `dates`, with the same merge-protection as daily dates.
